...
public class AWSClient {
	...
	AWSClient(){
		try {
    		credentials = new PropertiesCredentials(Local.class.getResourceAsStream(_propertiesFilePath));
	...
	// Download From S3
	public void S3download(String key, String filePath) throws IOException {
		  S3Object object = _s3Client.getObject(new GetObjectRequest(GLOBAL_BUCKET, key));
		  AWSClient.createFileFromInputStream(filePath, object.getObjectContent());				
	}

	//************************************************************************************************//
	// Create a file from S3 input stream                                                             //
	//************************************************************************************************//
	public static void createFileFromInputStream(String filePath, InputStream input) throws IOException
	{	
	...
	public List<Message> pollQueue(String fromQueue_name) {
		...
		int tries = 100;
			try{
				...
				System.out.println("No internet connection or queue deleted");
				if(--tries==0) throw e;
			}
		}
		return messages;
...
//	}
	...
	private void startWorker() {
		...
		riReq.setMinCount(1);
		riReq.setMaxCount(1);	
		riReq.setIamInstanceProfile(new IamInstanceProfileSpecification().withArn("arn:aws:iam::977810086336:instance-profile/eyar"));
		riReq.setUserData(getWorkerDataScript());
	...
	private void startManager() {
		...
		riReq.setMaxCount(1);	
		riReq.setIamInstanceProfile(new IamInstanceProfileSpecification().withArn("arn:aws:iam::977810086336:instance-profile/eyar"));
		riReq.setUserData(getManagerDataScript());
		RunInstancesResult riRes = _ec2Client.runInstances(riReq);
		List<String> instanceIdList = new ArrayList<>();
		List<Tag> tagsList = new ArrayList<>();
		String insId = riRes.getReservation().getInstances().get(0)
	...
	private static String getManagerDataScript() {
        ...
        lines.add("wget https://s3.amazonaws.com/hvljhbl/Manager.jar");
	...
	private static String getWorkerDataScript() {
        ArrayList<String> lines = new ArrayList<>();
        lines.add("#! /bin/bash -ex");
        lines.add("wget https://s3.amazonaws.com/hvljhbl/Worker.jar");
	...
	void TerminateAllWorkers() {
        ...
        List<Reservation> result = _ec2Client.describeInstances(req)
                .getReservations();
        for (Reservation reservation : result) {
            for (Instance instance : reservation.getInstances()) {
                if (instance.getState().getCode() == 16) { // running
                    for (Tag tag : instance.getTags()) {
                        ...
                        if (tag.getValue().equals("worker")) {
                            ...
                            _ec2Client.terminateInstances(new TerminateInstancesRequest(
                                            instanceIds));
                        }
                    }
                }
	...
	public void terminateAll() {
        ...
        for (Reservation reservation : result) {
            for (Instance instance : reservation.getInstances()) {
            	System.out.println("Currently open are " + reservation.getInstances().size() + " instances");
                if (instance.getState().getCode() == RUNNING || instance.getState().getCode() == PENDING) { // running
                            List<String> instanceIds = new ArrayList<>();
                            instanceIds.add(instance.getInstanceId());
                            _ec2Client.terminateInstances(new TerminateInstancesRequest(
                                            instanceIds));
	...
	public void getNumOfRunningInstances() {
		DescribeInstancesRequest dir = new DescribeInstancesRequest();
		List<Reservation> result = _ec2Client.describeInstances(dir)
				.getReservations();
		int count = 0;
	...
	public void setQVT(String queueName, int timeout) {
		String queue_url = _sqsClient.getQueueUrl(queueName).getQueueUrl();
		HashMap<String, String> attributesMap = new HashMap<String, String>();
		attributesMap.put("VisibilityTimeout", String.valueOf(timeout));
		_sqsClient.setQueueAttributes(queue_url,attributesMap);		
	}
}
